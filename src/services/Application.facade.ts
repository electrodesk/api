import { ApplicationReadDTO, CloseCommand, ApplicationExecCommand, ApplicationRegisterListenerCommand, ApplicationDispatchEventPayload, ApplicationRemoveListenerCommand, } from "@electrodesk/types/application"; // prettier-ignore
...
export class Application {
   ...
   */
  private handleElectronEvent(payload: ApplicationDispatchEventPayload): void {
    const eventHandler = this.eventRegistry.get(payload.event);
  ...
  private handleApplicationClosed(payload: unknown): void {
    const applicationData = payload as { id: string; application: string };
    if (applicationData.id !== this.application.uuid) {
      return;
    }
    this.clearAllEvents();
  }
